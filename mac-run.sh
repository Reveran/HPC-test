...
then
    ...
    do
        echo $mtx, $(./mac-matrix $mtx 1 --sequential) >> sequential.csv
...
if [ $1 = "--process" ]
then
    echo "Setting maximum shared memory to 2GB. Requesting root permissions"
    ...
    do
        ...
        for mtx in 480 960 1920 3840 7680
        do
            echo $thread, $mtx, $(./mac-matrix $mtx $thread --process) >> process.csv
            echo "\tDone Matrix" $mtx"x"$mtx"."
        done
...
then
    ...
    do
        ...
        for mtx in 480 960 1920 3840 7680
        do
            echo $thread, $mtx, $(./mac-matrix $mtx $thread --threading) >> threading.csv
            echo "\tDone Matrix" $mtx"x"$mtx"."
        done
        echo "Done" $thread threads.
    done
